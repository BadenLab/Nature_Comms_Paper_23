function STE_Full = STE_Full_FFF_Noise_norm_fn(stimulus_arr,trig_times_vec,spike_times_vec,length_spike_times,p)
% Calculate the Full Spike-Triggered Ensemble (STE) array
% Modified from STE_Full_fn_2

STE_Full = NaN(p.Num_STE_bins_norm,length_spike_times);

if p.Time_Choice == 1 % stimulus frames
    
    if p.Gap_Ind == 1 %  w/o gaps
        
        for i = 1:length_spike_times% was p.length_spike_times
            
            STE_index_loop          = sum((trig_times_vec-spike_times_vec(i))<0);
            loop_sample_indices_vec = mod(STE_index_loop-p.Num_STE_bins_norm:STE_index_loop-1,p.noise_length);
        ...
        for i = 1:length_spike_times
            ...
            STE_Full(:,i)  = stimulus_arr(STE_index_loop-p.Num_STE_bins_norm:STE_index_loop-1);
        end
...
elseif p.Time_Choice == 2 % define own time grid
    ...
    STE_index_vec = NaN(p.Num_STE_bins_norm,1);
    
    if p.Gap_Ind == 1 %  w/o gaps
        
        for i = 1:length_spike_times
            
            loop_sample_times_vec = spike_times_vec(i) + p.stim_timesample_vec_norm;
