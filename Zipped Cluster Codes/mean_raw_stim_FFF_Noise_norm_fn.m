function mean_raw_stim_arr = mean_raw_stim_FFF_Noise_norm_fn(stimulus_arr,trig_times_vec,p)
% Calculate the mean of the raw stimuli
% trig_times_vec: the vector input is 'trig_times_vec_trunc'

mean_raw_stim_arr_loop = zeros(p.Num_STE_bins_norm,1);
...
if p.Time_Choice == 1 || p.Mean_Stim_Choice  == 1 % stimulus frames
    ...
    if modNum_FinalFrames==0 % Every noise chunk is repeated a whole number of times
        ...
        for i = 1:p.Num_Raw_Stim_norm
            mean_raw_stim_arr_loop = mean_raw_stim_arr_loop + stimulus_arr(i:p.Num_STE_bins_norm+i-1);
        end
        
    else % modNum_FinalFrames~==0 % The final noise chunk does not contain a full set of frames
        
        for i = 1:p.Num_Raw_Stim_norm
            if modNum_FinalFrames >= p.Num_STE_bins_norm+i-1 % i
                mean_raw_stim_arr_loop = mean_raw_stim_arr_loop + p.Num_FNoise_rep_ceil*stimulus_arr(i:p.Num_STE_bins_norm+i-1);
            else % modNum_FinalFrames < p.Num_STE_bins_norm+i-1 % i
                mean_raw_stim_arr_loop = mean_raw_stim_arr_loop + (p.Num_FNoise_rep_ceil - 1)*stimulus_arr(i:p.Num_STE_bins_norm+i-1);
            end
        end
...
elseif p.Time_Choice == 2 && p.Mean_Stim_Choice  == 2 % define own time grid
    ...
    for i = 1:p.Num_Raw_Stim_norm
        ...
        loop_sample_times_vec = p.Num_Raw_Stim_t_vec_norm(i) + p.stim_timesample_vec_norm;
        loop_index_vec        = zeros(p.Num_STE_bins_norm,1);
...
if modNum_FinalFrames==0 % Every noise chunk is repeated a whole number of times
    ...
    mean_raw_stim_arr = mean_raw_stim_arr_loop/p.Num_Raw_Stim_norm;
    
else % modNum_FinalFrames~==0 % The final noise chunk does not contain a full set of frames
    
    if p.Time_Choice == 1 || p.Mean_Stim_Choice  == 1 % stimulus frames
        num_final_stim = modNum_FinalFrames - p.Num_STE_bins_norm + 1; % Number of raw stimuli in final noise chunk
        
        if num_final_stim > 0
            Mod_Num_Raw_Stim = p.Num_Raw_Stim_norm*(p.Num_FNoise_rep_ceil - 1) + num_final_stim;
        else
            Mod_Num_Raw_Stim = p.Num_Raw_Stim_norm*(p.Num_FNoise_rep_ceil - 1);
        end
        
    elseif p.Time_Choice == 2 && p.Mean_Stim_Choice  == 2 % define own time grid
        
        Mod_Num_Raw_Stim = loop_stim_count; % Was going to do the following for num_final_stim but incorrect, need to subtract fdifferent number but hard to determine: sum(p.Num_Raw_Stim_t_vec_norm<trig_times_vec(modNum_FinalFrames+1))- p.Num_STE_bins_norm + 1;% number of raw stim that can occur in  truncated final chunk
